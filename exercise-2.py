...
def contains_3_consecutive(list_of_nums):
    ...
    for i in range(len(list_of_nums) - 2):#loops through each number in the array until n-2 and returns true if match is found
        if (list_of_nums[i+1] == list_of_nums[i] + 1 and
            list_of_nums[i+2] == list_of_nums[i] + 2):
            return True
            # since the else condition is inside the loop it always print false if it misses the first iteration due to which we cannot proceed
    else:
        return False
        # line number 26 and 27 are causing the error
...
#assumption: both will print false